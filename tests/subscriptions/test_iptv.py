...
def subscription(iptv: mapping.SiteIndex) -> mapping.SubscriptionIdGuid:
    subscription_: mapping.SubscriptionId = iptv.subscriptions[0]
...
ATTRIBUTE = (
    mapping.subscription.stbid,
    mapping.subscription.stbmac,
    mapping.subscription.stbserial,
    mapping.subscription.tvdevicetype,
)
...
@allure.title("IPTV. Тест атрибута подключки")
...
def test_attribute(
    ...
    if attribute.is_sync:
        ...
        allure.dynamic.title(
            f"IPTV. Тест атрибута подключки '{attribute.description}'"
        )
