...
def subscription(lk: mapping.SiteIndex) -> mapping.SubscriptionIdGuid:
    subscription_: mapping.SubscriptionId = lk.subscriptions[0]
...
ATTRIBUTE = (
    mapping.subscription.cabinetlogin,
    mapping.subscription.cabinetpassword,
)
...
@allure.title("Личный кабинет. Тест атрибута подключки")
...
def test_attribute(
    ...
    if attribute.is_sync:
        ...
        allure.dynamic.title(
            f"Личный кабинет. Тест атрибута подключки '{attribute.description}'"
        )
