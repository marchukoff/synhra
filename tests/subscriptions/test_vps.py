...
def subscription(vps: mapping.SiteIndex) -> mapping.SubscriptionIdGuid:
    subscription_: mapping.SubscriptionId = vps.subscriptions[0]
...
ATTRIBUTE = (mapping.subscription.vps_, mapping.subscription.portnumbervps)
...
@allure.title("VPS. Тест атрибута подключки")
...
def test_attribute(
    ...
    if attribute.is_sync:
        ...
        allure.dynamic.title(
            f"VPS. Тест атрибута подключки '{attribute.description}'"
        )
